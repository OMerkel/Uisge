...
const showView = (view) => {
	...
	document.getElementById("app-header-title").textContent =
		view === "game" ? "Uisge" : view.charAt(0).toUpperCase() + view.slice(1);
};
...
const updateDifficultyBadge = (
	...
	badge.setAttribute(
		"aria-label",
		`Red ${south}, Yellow ${north}, profile ${profile}`,
	);
};
...
const readSettings = () => ({
	...
	deviceprofile:
		...
		"Auto",
	selectionmode:
		document.querySelector('input[name="selectionmode"]:checked')?.value ??
		"MustMove",
	resolveddeviceprofile: (() => {
		const selected =
			document.querySelector('input[name="deviceprofile"]:checked')?.value ??
...
const handleCellClick = (row, column) => {
	...
	const strictSelection =
		(state.settings.selectionMode ?? "MustMove") === "MustMove";
	const sourceActions = state.selectableActions.filter(
		(action) => action.from.row === row && action.from.column === column,
	);
	...
	const candidate = state.selectableActions.find(
		(action) =>
	...
	if (candidate) {
		sendToEngine("move", { action: candidate });
		return;
	}
	...
	if (
		...
		state.selectedSource.column === column
	) {
	...
	switch (data.request) {
		...
		case "human_to_move": {
			...
			if (wasAiMove(data.board, settingsSnapshot)) {
				pendingHumanTurnTimerId = setTimeout(
					dispatchHumanTurn,
					AI_MOVE_PAUSE_MS,
				);
			} else {
		...
		case "ai_to_move": {
			...
			const delay = wasAiMove(data.board, settingsSnapshot)
				? AI_MOVE_PAUSE_MS
				: 0;
...
store.subscribe((state) => {
	...
	if (renderer && state.board) {
		const allowReselect =
			(state.settings.selectionMode ?? "MustMove") === "Flexible";
		renderer.render(
			state.board,
...
const STORAGE_KEY = "uisge_user_settings";
	...
	try {
		const settings = {
			...
			secondplayer:
				...
				"Human",
			difficultysouth:
				document.querySelector('input[name="difficultysouth"]:checked')
					?.value ?? "Medium",
			difficultynorth:
				document.querySelector('input[name="difficultynorth"]:checked')
					?.value ?? "Medium",
			deviceprofile:
				document.querySelector('input[name="deviceprofile"]:checked')?.value ??
				"Auto",
	...
	try {
		...
		if (settings.firstplayer) {
			const el = document.querySelector(
				`input[name="firstplayer"][value="${settings.firstplayer}"]`,
			);
			if (el) el.checked = true;
		}
		if (settings.secondplayer) {
			const el = document.querySelector(
				`input[name="secondplayer"][value="${settings.secondplayer}"]`,
			);
			if (el) el.checked = true;
		}
		if (settings.difficultysouth) {
			const el = document.querySelector(
				`input[name="difficultysouth"][value="${settings.difficultysouth}"]`,
			);
			if (el) el.checked = true;
		}
		if (settings.difficultynorth) {
			const el = document.querySelector(
				`input[name="difficultynorth"][value="${settings.difficultynorth}"]`,
			);
			if (el) el.checked = true;
		}
		if (settings.deviceprofile) {
			const el = document.querySelector(
				`input[name="deviceprofile"][value="${settings.deviceprofile}"]`,
			);
			if (el) el.checked = true;
		}
		if (settings.selectionmode) {
			const el = document.querySelector(
				`input[name="selectionmode"][value="${settings.selectionmode}"]`,
			);
			if (el) el.checked = true;
		}
	} catch (error) {
...
const wireUI = () => {
	...
	document.getElementById("nav-new")?.addEventListener("click", () => {
		cancelPendingTurnTimers();
		// Always sync from current options controls before restart so badge and
		// engine settings cannot drift when a game is restarted from the menu.
	...
	document
		.querySelectorAll(
			'input[name="firstplayer"], input[name="secondplayer"], input[name="difficultysouth"], input[name="difficultynorth"], input[name="deviceprofile"], input[name="selectionmode"]',
		)
		.forEach((input) => {
