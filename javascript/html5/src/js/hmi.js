...
const handleCellClick = (row, column) => {
  ...
  const candidate = state.selectableActions.find((action) =>
...
store.subscribe((state) => {
  ...
  if (renderer && state.board) {
    ...
    renderer.render(state.board, state.selectableActions, state.selectedSource, allowReselect);
  }
...
const wireUI = () => {
  ...
  document.querySelectorAll('input[name="firstplayer"], input[name="secondplayer"], input[name="difficultysouth"], input[name="difficultynorth"], input[name="deviceprofile"], input[name="selectionmode"]').forEach((input) => {
