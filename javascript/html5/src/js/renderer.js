...
const CROWN_PATHS = [
	{
		style:
			"fill:#000000;fill-opacity:1;stroke:#333333;stroke-width:8;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1",
	...
	{
		style:
			"fill:#ffffff;fill-opacity:1;stroke:#333333;stroke-width:8;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1",
	...
	{
		style:
			"fill:none;stroke:#000000;stroke-width:16;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none",
	...
	{
		style:
			"fill:#000000;fill-opacity:1;stroke:#333333;stroke-width:8;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none",
...
export const createRenderer = (container, onCellClick) => {
	...
	const statusText = svgEl("text", {
		...
		"text-anchor": "middle",
		style:
			"font:700 72px/1 system-ui,sans-serif;fill:#f8fafc;stroke:#1f2937;stroke-width:2;",
	});
	...
	const handlers = Array.from({ length: ROWS }, () =>
	...
	const render = (
		...
		const destinationSet = new Set(
			selectableActions
				.filter(
					(a) =>
						selectedFrom &&
						a.from.row === selectedFrom.row &&
						a.from.column === selectedFrom.column,
				)
		...
		for (let row = 0; row < ROWS; row++) {
			for (let col = 0; col < COLUMNS; col++) {
				...
				const isLastMoveSource =
					latest && latest.from.row === row && latest.from.column === col;
				cell.sourceIndicator.setAttribute(
					"display",
					isLastMoveSource ? "block" : "none",
				);
				...
				if (piece === EMPTY) {
					...
					if (isSelected) {
						...
						cell.disc.style.filter =
							"drop-shadow(0 0 18px rgba(34,197,94,0.95))";
						...
						cell.disc.style.filter =
							"drop-shadow(0 0 12px rgba(248,250,252,0.95))";
					} else {
					...
					if (isKingPiece(piece)) {
						...
						cell.crown.style.filter =
							piece === SOUTH_KING ? CROWN_FILTER_SOUTH : CROWN_FILTER_NORTH;
					} else {
						cell.crown.setAttribute("display", "none");
		...
		if (boardState.winner === 0) statusText.textContent = "Red wins";
			...
			statusText.textContent =
				boardState.active === 0 ? "Red to move" : "Yellow to move";
