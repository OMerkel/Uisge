...
const applyActionToGrid = (grid, action) => {
	...
	next[action.to.row][action.to.column] =
		action.type === "jump" ? toggleFace(piece) : piece;
	return next;
};
	...
	for (let row = 0; row < ROWS; row++) {
		for (let col = 0; col < COLUMNS; col++) {
			...
			for (const [dr, dc] of ORTHOGONAL_DIRS) {
				...
				if (grid[midRow][midCol] === EMPTY || grid[toRow][toCol] !== EMPTY)
					continue;
...
export const getActions = (board) => {
	if (typeof process !== "undefined" && process.env.NODE_ENV !== "production") {
		console.assert(
			board.grid &&
				board.grid.length === ROWS &&
				board.grid[0].length === COLUMNS,
...
export const doAction = (board, action) => {
	...
	return {
		...
		winningLine:
			winner !== null ? [{ row: legal.to.row, column: legal.to.column }] : null,
