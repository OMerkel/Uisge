...
describe("controller worker message handling", () => {
	...
	it("resolves no-legal-move as immediate loss when AI returns null", async () => {
		...
		const requests = posted.map((m) => m.request);
		expect(
			requests.filter((r) => r === "redraw").length,
	...
	it("does not overwrite winner when action_by_ai arrives after game is already over", async () => {
		const posted = [];
		const listeners = new Map();
		...
		expect(redraws[0].board.winner).toBe(1);
	});
