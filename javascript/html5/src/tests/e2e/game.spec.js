...
const getSelectedPieceCenter = (page) =>
	page.evaluate(() => {
		const pieceCircles = [
			...document.querySelectorAll("#board svg circle"),
		].filter((c) => {
			const fill = c.getAttribute("fill");
			return fill === "#dc2626" || fill === "#facc15";
		});
		const selected = pieceCircles.find(
			(c) => c.getAttribute("stroke") === "#22c55e",
		);
		return selected
...
const clickOverlayAt = async (page, cell) => {
	await page.evaluate((target) => {
		...
		const hit = overlays.find(
			(c) =>
				c.getAttribute("cx") === target.cx &&
				c.getAttribute("cy") === target.cy,
		);
...
const getVisibleCrownCount = (page) =>
	page.evaluate(() => {
		const crowns = [
			...document.querySelectorAll('#board svg [data-role="crown"]'),
		];
		return crowns.filter((group) => group.getAttribute("display") !== "none")
			.length;
	});
...
const playFirstLegalMove = async (page) => {
	...
	if (destinations.length === 0)
		throw new Error("No selectable destination found");
...
test.describe("Navigation", () => {
	test("Rules, Options, About are standalone pages without game board", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("Back from rules/about returns to game view", async ({ page }) => {
		await page.goto("/");
	...
	test("New Game from options applies settings and updates badge", async ({
		page,
	}) => {
		await page.goto("/");
		await page.locator("#btn-menu").click();
	...
	test("New Game keeps both AI Hard difficulties in badge", async ({
		page,
	}) => {
		await page.goto("/");
		await page.locator("#btn-menu").click();
	...
	test("OK from options returns to game view", async ({ page }) => {
		await page.goto("/");
		await page.locator("#btn-menu").click();
		await page.locator("#nav-options").click();
		await expect(page.locator("#view-options")).toBeVisible();
...
test.describe("Options", () => {
	test("player options are present", async ({ page }) => {
		...
		await expect(
			page.locator('input[name="firstplayer"][value="Human"]'),
		).toBeChecked();
		await expect(
			page.locator('input[name="secondplayer"][value="Human"]'),
		).toBeChecked();
		await expect(
			page.locator('input[name="difficultysouth"][value="Medium"]'),
		).toBeChecked();
		await expect(
			page.locator('input[name="difficultynorth"][value="Medium"]'),
		).toBeChecked();
		await expect(
			page.locator('input[name="deviceprofile"][value="Auto"]'),
		).toBeChecked();
		await expect(
			page.locator('input[name="selectionmode"][value="MustMove"]'),
		).toBeChecked();
		await expect(page.locator("#device-profile-hint")).toHaveText(
			/Auto currently resolves to (Desktop|Mobile)\./,
		);
	});

	test("changing red and yellow difficulties updates header badge", async ({
		page,
	}) => {
		await page.goto("/");
		await page.locator("#btn-menu").click();
	...
	test("badge shows human for human side and difficulty for AI side", async ({
		page,
	}) => {
		await page.goto("/");
		await page.locator("#btn-menu").click();
	...
	test("closing side panel from options applies settings and returns to game view", async ({
		page,
	}) => {
		await page.goto("/");
		await page.locator("#btn-menu").click();
	...
	test("changing active side to AI triggers immediate AI thinking via sync", async ({
		page,
	}) => {
		await page.goto("/");
		await expect(page.locator("#board svg text").first()).toHaveText(
			"Red to move",
		);
		...
		await expect(page.locator("#board svg text").first()).not.toHaveText(
			"Red to move",
			{ timeout: 10_000 },
		);
	});

	test("switching AI back to human quickly keeps human turn", async ({
		page,
	}) => {
		await page.goto("/");
		await expect(page.locator("#board svg text").first()).toHaveText(
...
test.describe("Board interaction", () => {
	...
	test("default selection mode keeps selected source until moved", async ({
		page,
	}) => {
		await page.goto("/");
		await waitForClickableAction(page);
		...
		expect(
			afterSelect.some((c) => c.cx === sources[0].cx && c.cy === sources[0].cy),
		).toBe(false);
	...
	test("flexible selection mode allows deselect and reselect", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("new game in human mode has immediate selectable sources", async ({
		page,
	}) => {
		await page.goto("/");
...
test.describe("Accessibility", () => {
	test("header button has accessible label", async ({ page }) => {
		await page.goto("/");
		await expect(page.locator("#btn-menu")).toHaveAttribute(
			"aria-label",
			/menu/i,
		);
	});
	...
	test("panel separators use semantic separator role", async ({ page }) => {
		await page.goto("/");
		await expect(
			page.locator('#side-panel .panel-divider[role="separator"]'),
		).toHaveCount(2);
	});
});
